...
// Temporary fail-safe containment switch. Protected functionality opens only
// when MAINTENANCE_MODE is explicitly set to the string "false".
const MAINTENANCE_PAGE = path.join(__dirname, 'maintenance.html');
const maintenanceActive = () => {
  try {
    return String(process.env.MAINTENANCE_MODE || '').trim().toLowerCase() !== 'false';
  } catch (e) {
    return true;
  }
};
...
const sendMaintenance = (res) => send(res, 503, {
  error: 'MandIMMO is temporarily unavailable while account security is being finalised.'
}, maintenanceHeaders);
const showMaintenancePage = (res) => {
  let page;
  try { page = fs.readFileSync(MAINTENANCE_PAGE, 'utf8'); }
  catch (e) { page = '<!doctype html><html lang="fr"><meta charset="utf-8"><title>MandIMMO — Ouverture prochaine</title><h1>MandIMMO évolue — ouverture prochaine.</h1><p>Les nouvelles inscriptions et la collecte de contacts sont temporairement suspendues.</p></html>'; }
  res.writeHead(503, { 'Content-Type': 'text/html; charset=utf-8', 'X-Robots-Tag': 'noindex, nofollow', ...maintenanceHeaders });
  res.end(page);
};
...
const server = http.createServer(async (req, res) => {
  ...
  // Health checks must remain available to Render and contain no application data.
  if (req.method === 'GET' && u.pathname === '/health') {
    return send(res, 200, { ok: true, maintenance: maintenanceActive() });
  }

  // This is intentionally the first application gate: before authentication,
  // request-body parsing, database calls, email, Stripe, or visit logging.
  if (maintenanceActive()) {
    // Close the complete API namespace for every HTTP method. This is checked
    // before auth, request-body parsing, DB, email, Stripe, or visit logging.
    if (u.pathname === '/api' || u.pathname.startsWith('/api/')) {
      return sendMaintenance(res);
    }
    // Fail closed for every other route too, including current and future
    // non-API write paths, forgotten static pages, documents, and unknown URLs.
    // The holding page is self-contained, so no application assets need through.
    return showMaintenancePage(res);
  }
